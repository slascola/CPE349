...
public class DiGraph {
            ...
            if (graph[u - 1] != null) {
                for (int v = 0; v < graph[u-1].size(); v++) {
                    if (VA[graph[u-1].get(v) - 1].distance == -1) {
                        VA[graph[u-1].get(v) - 1].distance = VA[u-1].distance + 1;
    ...
    private class TreeNode {
        int vertexNum = 0;
        LinkedList<TreeNode> childList = new LinkedList<>();
    }

    private TreeNode buildTree(int s) {
        VertexInfo[] path = BFS(s);
        TreeNode[] vertices = new TreeNode[path.length];
        ...
        for (int i = 0; i < path.length; i++) {
            if (path[i].predecessor != -1) {
                vertices[path[i].predecessor - 1].childList.add(vertices[i]);
            }
        }
    ...
    public void printTree(int s) {
        TreeNode source = buildTree(s);
        System.out.println("Printing Tree");
        printTree(source, "");
    }

    private void printTree(TreeNode source, String indent) {
        System.out.println(indent + source.vertexNum);
        for (int i = 0; i < source.childList.size(); i++) {
            if (i== 0) {
                indent+="    ";
            }
            printTree(source.childList.get(i), indent);
        }
    }
}
